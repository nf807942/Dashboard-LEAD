...
export class ApiService {
  ...
  get(module: string, endpoint: string, id?: number, options: any = {}): Observable<any> {
    options = {...options, withCredentials: true};

    return this.http.get(this.url(module, endpoint, id), options).pipe(
      this.errorCatching,
      catchError(() => of([]))
    );
  }

  post(module: string, endpoint: string, data: any, id?: number, options: any = {}): Observable<any> {
    options = {...options, withCredentials: true};

    return this.http.post(this.url(module, endpoint, id), data, options).pipe(this.errorCatching);
  }
